...
def _title_card(title, subtitle, path):
    ...
    centered(title, h * 0.37, title_font, (248, 250, 255))
    centered(subtitle, h * 0.53, sub_font, (255, 207, 91))
...
def _animated_clip(card_path, audio, duration):
    ...
    # Tiny scale drift creates movement without changing the readable layout.
    animated = base.resize(lambda t: 1.0 + 0.025 * min(1.0, t / max(duration, 0.1))).set_position("center")
    return CompositeVideoClip([animated], size=(config.VIDEO_WIDTH, config.VIDEO_HEIGHT)).set_duration(duration).set_audio(audio)
...
def build_video(title=None, out_path=None, words_path=None):
    ...
    audio_items.append(("Welcome. We will learn each Italian word through its English meaning, a clear explanation, and a memorable visual example.", intro_audio_path))
    for index, lesson in enumerate(lessons, 1):
        audio_items.append((_word_narration(lesson), os.path.join(work_dir, f"word_{index:02d}.mp3")))
    audio_items.append(("Great work. Pause here, repeat the Italian words aloud, and try using them in your own sentences.", outro_audio_path))
    ...
    for index, lesson in enumerate(lessons, 1):
        ...
        audio = AudioFileClip(os.path.join(work_dir, f"word_{index:02d}.mp3"))
        opened.append(audio)
